...
FAST5FILES=$(ls $HERE/FAST5/*.fast5)
...
echo "found $NBFAST5FILES files with $EXT extension in $HERE/FAST5)" >>  $LOG
cd $HERE
...
JOBFILE=$HERE/JOBS/pipeline${FILE}.job
...
#	echo "bonito basecaller --use_openvino --device cpu $MODEL $HERE/FAST5 | bgzip -@ $THREADS > $HERE/FASTQ/tmp${FILE}_$SUBDIR.fastq.gz" >> $JOBFILE
echo "bonito basecaller --use_openvino --device cpu $MODEL $HERE/FAST5 | bgzip -@ $THREADS > $HERE/FASTQ/tmp${FILE}.fastq.gz" >> $JOBFILE
...
echo "porechop-runner.py -t $THREADS -i $HERE/FASTQ/tmp${FILE}.fastq.gz -o $HERE/FASTQ/${FILE}.fastq.gz">>$JOBFILE
...
echo "minimap2 -a -x map-ont -t $THREADS $REFS/chm13v2.0.mmi $HERE/FASTQ/${FILE}.fastq.gz | samtools sort -@ $THREADS -o $HERE/BAM/${FILE}.bam" >>$JOBFILE
